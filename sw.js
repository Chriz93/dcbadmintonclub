...
self.addEventListener('push', e => {
  ...
  e.waitUntil(self.registration.showNotification(d.title || 'Maplewood League', {
    body: d.body || '', icon: '/dcbadmintonclub/icon-192.png', badge: '/dcbadmintonclub/icon-192.png',
    tag: d.tag || 'maplewood-vote', renotify: true, data: { url: d.url || '/dcbadmintonclub/' }, actions: d.actions || []
  }));
});
self.addEventListener('notificationclick', e => {
  e.notification.close();
  let url = (e.notification.data && e.notification.data.url) || '/dcbadmintonclub/';
